...
echo | add-apt-repository ppa:mozillateam/ppa || { log "Failed to add Firefox ESR repository"; exit 1; }
apt update || { log "Failed to update package lists"; exit 1; }
apt install firefox-esr -y || { log "Failed to install Firefox ESR"; exit 1; }
apt-get install --fix-broken -y || { log "Failed to fix broken dependencies after Firefox ESR installation"; exit 1; }
